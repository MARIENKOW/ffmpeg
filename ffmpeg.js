...
const WATERMARK_SCALE = process.env.WATERMARK_SCALE || '1';
...
const addWatermark = (inputPath, outputPath) => {
  return new Promise((resolve, reject) => {
    buildCommand(inputPath, outputPath)
      ...
      .on('end',   ()    => { console.log('[ffmpeg:full] done'); resolve(outputPath); })
      .on('error', (err) => { console.error('[ffmpeg:full] error:', err.message); reject(err); })
      .run();
  });
};
...
const addWatermarkAndTrim = (inputPath, outputPath, durationSec) => {
  return new Promise((resolve, reject) => {
    buildCommand(inputPath, outputPath, [`-t ${durationSec}`])
      ...
      .on('end',   ()    => { console.log('[ffmpeg:short] done'); resolve(outputPath); })
      .on('error', (err) => { console.error('[ffmpeg:short] error:', err.message); reject(err); })
      .run();
  });
};
